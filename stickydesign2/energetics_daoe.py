...
class EnergeticsDAOE(object):
    ...
    def gse_matching(self, seqs):
        ps = seqs.nnseq

        # In both cases here, the energy we want is the NN binding energy of
        # each stack,
        if isinstance(seqs, sc.EndPairArrayDT):
            dcorr = np.zeros(ps.shape[:-1])
            if self.danglecorr:
                dcorr += -self.dangle3dG[ps[..., 0]] - self.dangle3dG[ps.rc[..., 0]]
            if self.coaxparams:
                dcorr += self.coaxddG[ps[..., 0]] + self.coaxddG[ps.rc[..., 0]]
        elif isinstance(seqs, sc.EndPairArrayTD):
            dcorr = np.zeros_like(self.dangle5dG[ps[:, -1]])
            if self.danglecorr:
                dcorr += (-self.dangle5dG[ps[:, -1]] -
                          self.dangle5dG[ps.rc[:, -1]])
            if self.coaxparams:
                dcorr += self.coaxddG[ps[:, -1]] + self.coaxddG[ps.rc[:, -1]]
        else:
            raise NotImplementedError
        return -(np.sum(self.nndG[ps], axis=-1) + self.initdG + dcorr)

    def gse_all(self, seqs1, seqs2, debug=False, forcemulti=False):
        ...
        seqs1, seqs2 = np.broadcast_arrays(seqs1, seqs2, subok=True)
        ...
        if isinstance(seqs1, sc.EndArrayTD):
            ...
            s2rl = np.concatenate((seqs2.rc, seqs1[..., -1:]),
                                  axis=-1).view(sc.EndArrayTD).nnseq
        elif isinstance(seqs1, sc.EndArrayDT):
            s1_end = s1[..., 1:]
            s2_end_rc = s2r[..., 0:-1]
            s2rl = np.concatenate((seqs1[..., 0:1], seqs2.rc),
                                  axis=-1).view(sc.EndArrayDT).nnseq
            s1l = np.concatenate((seqs1, seqs2.rc[..., -1:]),
                                 axis=-1).view(sc.EndArrayDT).nnseq
        else:
            raise NotImplementedError
            ...
            if offset > 0:
                if isinstance(seqs1, sc.EndArrayTD):
                    ...
                    intmm = -self.intmmdG_5335[s1_end[..., :-offset] * 16
                                               + s2_end_rc[..., offset:]]
                if isinstance(seqs1, sc.EndArrayDT):
                    ens = (s1_end[..., offset:] == s2_end_rc[..., :-offset]) * (
                        -self.nndG[s1_end[..., offset:]])
                    ens[..., 0] += (ens[..., 0] != 0) * (
...
class EnergeticsDAOEC(EnergeticsDAOE):
    ...
    def __init__(self, seqlen, *args, **kwargs):
        EnergeticsDAOE.__init__(self, *args, **kwargs)
        # FIXME: these can be smaller
        self._storeTD = np.ma.masked_all((4**((seqlen+1)*2),))
        self._storeDT = np.ma.masked_all((4**((seqlen+1)*2),))
        self._multarS = 4**np.arange(seqlen, -1, -1)
        self._multarB = 4**(seqlen+1) * self._multarS
        self.seqlen = seqlen

    def gse_all(self, seqs1, seqs2, forcemulti=False):
        assert seqs1.shape[-1] == seqs2.shape[-1] == self.seqlen+1
        assert seqs1.__class__ == seqs2.__class__
        if isinstance(seqs1, sc.EndArrayTD):
            store = self._storeTD
        if isinstance(seqs1, sc.EndArrayDT):
            store = self._storeDT
        ...
        todo = store.mask[indexes].nonzero()

        if multi and len(seqs1[todo[0]]) > 0:
            store[indexes[todo]] = EnergeticsDAOE.gse_all(self,
                                                                 seqs1[todo[0], 0, :],
                                                                 seqs2[0, todo[1], :])
        ...
        assert np.all(~store[indexes].mask)
            
        return store[indexes].filled(np.nan)
